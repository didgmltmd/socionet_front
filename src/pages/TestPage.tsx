...
const contentMap: Record<string, { title: string; text: string }> = {
  '검사신청': {
    title: '검사신청',
    text: `SOCIONET 집단검사를 신청하기 위한 선행조건 안내
    
① 검사의 신청에 필요한 소정의 초급 교육을 수료한(선이수) 이후에 신청이 가능합니다.
② 선검사 신청 이후, 후이수교육 조건으로 검사신청이 가능합니다.
...
온라인 교육을 이수하고 싶은 사람은 반드시 “로그인”을 해야 한다. 로그인을 하기 위해서는 ①회원가입을 해야 한다. ②회원가입을 한 이후에 한국SOCIONET연구소는 회원가입자를 확인하고 적절한 교육이수자인지를 심사하여 그 결과를 유무선으로 반드시 통보한다. ③한국SOCIONET연구소의 통보가 이루어진 다음부터 자신의 “메일주소”와 “비번”을 통해서 로그인이 가능해지고 온라인 교육을 받을 수 있다.`,
  },
  '초급 SOCIONET': {
    title: '초급 SOCIONET',
  ...
  '고급 SOCIONET': {
    title: '고급 SOCIONET',
    text: `고급 SOCIONET교육 과정은 박사과정 이상의 학생이 “SOCIONET 자체”를 연구의 대상으로 삼았을 때 필요한 교육을 제공하는 과정이다. 시간 제한은 없다. 이 과정은 박사과정 이상의 학생과 연구소 측의 구두 또는 문서화된 약속으로 교육의 범위를 설정한다. 학위 과정에서 설문지로 SOCIONET 검사도구가 확정되었을 때에는 설문지 비용의 discount 과정도 여기에 포함된다. 나머지 문제는 SOCIONET연구소와 합의한다. 온라인으로 고급 SOCIONET과정을 개설하지 않는다.`,
  },
  '일반강사과정': {
    title: '일반강사 과정',
