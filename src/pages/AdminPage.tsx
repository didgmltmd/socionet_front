...
export default function AdminPage() {
  ...
  const handleApprove = async (userId: string) => {
    try {
      ...
      alert(error instanceof Error ? error.message : '?πÏù∏ Ï≤òÎ¶¨???§Ìå®?àÏäµ?àÎã§.')
    }
  }
  ...
  const handleReject = async (userId: string) => {
    try {
      ...
      alert(error instanceof Error ? error.message : 'Í±∞Ï†à Ï≤òÎ¶¨???§Ìå®?àÏäµ?àÎã§.')
    }
  }
  ...
  const handleSaveRole = async (userId: string) => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Í∂åÌïú ?Ä?•Ïóê ?§Ìå®?àÏäµ?àÎã§.')
    }
  }
  ...
  const handleDeleteUser = async (userId: string) => {
    try {
      ...
      alert(error instanceof Error ? error.message : '??†ú???§Ìå®?àÏäµ?àÎã§.')
    }
  }
...
function EducationManagement() {
  ...
  const handleUpload = async () => {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      alert('Supabase ?òÍ≤ΩÎ≥Ä?òÍ? ?§Ï†ï?òÏ? ?äÏïò?µÎãà??')
      return
    }
    ...
    try {
      ...
      setUploadMessage('?ÖÎ°ú?úÎêò?àÏäµ?àÎã§!')
      ...
      const message =
        error instanceof Error
          ? error.name === 'AbortError'
            ? '?∏ÏΩî???úÍ∞Ñ??Ï¥àÍ≥º?òÏóà?µÎãà?? ?§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.'
            : error.message
          : '?ÅÏÉÅ ?±Î°ù???§Ìå®?àÏäµ?àÎã§.'
    ...
    fetchVideo(video.id)
      .then(({ video: detail }) => {
        if (detail.signedUrl) {
          ...
          setPreviewError('?ÅÏÉÅ ÎØ∏Î¶¨Î≥¥Í∏∞Î•?Î∂àÎü¨?????ÜÏäµ?àÎã§.')
        }
      })
      .catch(() => setPreviewError('?ÅÏÉÅ ÎØ∏Î¶¨Î≥¥Í∏∞???§Ìå®?àÏäµ?àÎã§.'))
...
function PostManagement() {
  ...
  const formatPlainTextToHtml = (raw: string) => {
    ...
    const parseScheduleTable = (tableLines: string[]) => {
      const headerIndex = tableLines.findIndex((line) =>
        ['?†Ïßú', '?úÍ∞Ñ', 'Î∞úÌëú??, 'Ï∞∏Í???, '?∏Ï†ï?ôÌöå'].includes(line),
      )
    ...
    while (index < lines.length) {
      ...
      if (line.startsWith('*????) || line.startsWith('?≤Ïùº ??)) {
  ...
  const insertChecklist = (mode: 'create' | 'edit') => {
    ...
    insertBlock(
      editorRef,
      '<ul><li data-block-id="__BLOCK_ID__" data-placeholder="Ï≤¥ÌÅ¨ ??™©">??</li></ul><p><br></p>',
      htmlRef,
    )
  }
      ...
      <div className="mb-3 space-y-2 rounded-2xl border border-gray-100 bg-gray-50/80 p-2">
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm">
            <span className="text-[11px] font-semibold text-gray-500">Í∏ÄÍº?/span>
            <select
              onChange={(event) => applyFontFamily(mode, event.target.value)}
              className="rounded-md border border-gray-200 bg-white px-2 py-1 text-xs"
              defaultValue="ÎßëÏ? Í≥†Îîï"
            >
              <option value="ÎßëÏ? Í≥†Îîï">ÎßëÏ? Í≥†Îîï</option>
              <option value="?òÎàîÍ≥†Îîï">?òÎàîÍ≥†Îîï</option>
              <option value="Íµ¥Î¶º">Íµ¥Î¶º</option>
              <option value="Î∞îÌÉï">Î∞îÌÉï</option>
            </select>
            <select
          ...
          <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm">
            <span className="text-[11px] font-semibold text-gray-500">?úÏãù</span>
            <button
              type="button"
            ...
            <button
              ...
              onClick={() =>
                insertBlock(
                  ...
                  '<h2 data-block-id="__BLOCK_ID__" data-placeholder="?úÎ™©"></h2><p><br></p>',
            ...
            <button
              ...
              onClick={() =>
                insertBlock(
                  mode === 'create' ? contentEditorRef : editingEditorRef,
                  '<h3 data-block-id="__BLOCK_ID__" data-placeholder="?åÏ†úÎ™?></h3><p><br></p>',
                  mode === 'create' ? contentHtmlRef : editingHtmlRef,
                )
              }
              className="rounded-md border border-gray-200 px-2 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50 hover:cursor-pointer"
          ...
          <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm">
            <span className="text-[11px] font-semibold text-gray-500">?âÏÉÅ</span>
          ...
          <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm">
            <span className="text-[11px] font-semibold text-gray-500">?ïÎ†¨</span>
            <button
              type="button"
          ...
          <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm">
            <span className="text-[11px] font-semibold text-gray-500">?§Ïó¨?∞Í∏∞</span>
            <button
              type="button"
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm">
            <span className="text-[11px] font-semibold text-gray-500">Î™©Î°ù</span>
            <button
              type="button"
              onClick={() =>
                insertBlock(
                  mode === 'create' ? contentEditorRef : editingEditorRef,
                  '<ul><li data-block-id="__BLOCK_ID__" data-placeholder="Î™©Î°ù ??™©"></li></ul><p><br></p>',
                  mode === 'create' ? contentHtmlRef : editingHtmlRef,
                )
              }
              className="rounded-md border border-gray-200 px-2 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50 hover:cursor-pointer"
            ...
            <button
              ...
              onClick={() =>
                insertBlock(
                  mode === 'create' ? contentEditorRef : editingEditorRef,
                  '<ol><li data-block-id="__BLOCK_ID__" data-placeholder="Î™©Î°ù ??™©"></li></ol><p><br></p>',
                  mode === 'create' ? contentHtmlRef : editingHtmlRef,
                )
              }
              className="rounded-md border border-gray-200 px-2 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50 hover:cursor-pointer"
          ...
          <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-sm">
            <span className="text-[11px] font-semibold text-gray-500">?ΩÏûÖ</span>
            <button
              type="button"
      ...
      return `<tr>${Array.from({ length: clampedCols }, (_col, colIndex) => {
        const cellAttrs =
          rowIndex === 0 && colIndex === 0
            ? ' data-block-id="__BLOCK_ID__" data-placeholder="?¥Ïö©"'
            : ' data-placeholder="?¥Ïö©"'
  ...
  const handleLinkSubmit = () => {
    ...
    if (selection && selection.toString().trim()) {
      ...
      insertBlock(
        editorRef,
        `<p><a href="${url}" target="_blank" rel="noopener noreferrer" data-block-id="__BLOCK_ID__" data-placeholder="ÎßÅÌÅ¨ ?çÏä§??></a></p>`,
        htmlRef,
      )
    }
    setActiveModal(null)
  ...
  const handleImageUpload = async (file: File, onInsert: (url: string) => void) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : '?¥Î?ÏßÄ ?ÖÎ°ú??URL??ÎßåÎì§ÏßÄ Î™ªÌñà?µÎãà??'
    ...
    if (!uploadResponse.ok) {
      ...
      throw new Error(`?¥Î?ÏßÄ ?ÖÎ°ú?úÏóê ?§Ìå®?àÏäµ?àÎã§.${suffix}`)
    }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl bg-white p-6 shadow-md">
        <h3 className="mb-4 text-xl font-bold text-gray-800">Í≤åÏãúÍ∏Ä ?±Î°ù</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="mb-2 block text-sm font-bold text-gray-700">
              ?úÎ™©
            ...
            <input
              ...
              onChange={(event) => setTitle(event.target.value)}
              className="w-full rounded-lg border-2 border-gray-200 px-4 py-2"
              placeholder="Í≤åÏãúÍ∏Ä ?úÎ™©???ÖÎ†•?òÏÑ∏??"
            />
          </div>
          <div>
            <label className="mb-2 block text-sm font-bold text-gray-700">
            ...
            <select
              ...
              className="w-full rounded-lg border-2 border-gray-200 px-4 py-2"
            >
              <option value="NOTICE">Í≥µÏ??¨Ìï≠</option>
              <option value="ACTIVITY">Ïª§Î??àÌã∞ ?úÎèô</option>
            </select>
          </div>
          <div className="md:col-span-2">
            <label className="mb-2 block text-sm font-bold text-gray-700">
            ...
            <input
              ...
              onChange={async (event) => {
                ...
                try {
                  ...
                  alert(error instanceof Error ? error.message : '?¥Î?ÏßÄ ?ÖÎ°ú?úÏóê ?§Ìå®?àÏäµ?àÎã§.')
                }
              }}
            />
          </div>
          <div className="flex items-center gap-2">
            <input
              ...
              className="h-4 w-4"
            />
            <span className="text-sm text-gray-700">?ÅÎã® Í≥†Ï†ï</span>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-xl bg-white p-6 shadow-md">
        <h3 className="mb-4 text-xl font-bold text-gray-800">Í≤åÏãúÍ∏Ä Î™©Î°ù</h3>
        {isLoading ? (
          <p className="text-sm text-gray-500">Î∂àÎü¨?§Îäî Ï§?..</p>
        ) : posts.length === 0 ? (
          <p className="text-sm text-gray-500">?±Î°ù??Í≤åÏãúÍ∏Ä???ÜÏäµ?àÎã§.</p>
        ) : (
          <div className="space-y-3">
            {posts.map((post) => (
              <div
      ...
      {editingPostId && (
        ...
        >
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-800">Í≤åÏãúÍ∏Ä ?òÏ†ï</h3>
              <button
                type="button"
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h4 className="text-lg font-bold text-gray-800">
                {activeModal.type === 'link' ? 'ÎßÅÌÅ¨ Ï∂îÍ?' : '??ÎßåÎì§Í∏?}
              </h4>
              <button
                type="button"
                onClick={() => {
